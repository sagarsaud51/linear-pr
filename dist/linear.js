...
async function setupWithApiKey() {
    ...
    try {
        ...
        console.error(chalk.red('Error connecting to Linear:'), error instanceof Error ? error.message : 'Invalid token');
...
async function setupWithOAuth() {
    ...
    try {
        ...
        console.error(chalk.red('Error connecting to Linear:'), error instanceof Error ? error.message : 'Authentication failed');
        throw new Error('Failed to connect to Linear');
    }
}
/**
 ...
 */
async function performOAuth2Flow(clientId, clientSecret) {
 ...
 */
export async function getTask(taskId, checkAssignment = false) {
    const client = await getLinearClient();
    try {
 ...
 */
export async function isTaskAssignedToCurrentUser(taskId) {
    try {
 ...
 */
export async function attachPRToTask(taskId, prUrl) {
    const client = await getLinearClient();
    try {
        // Get the task
 ...
 */
export async function getAssignedTasks(limit = 50) {
    const client = await getLinearClient();
    try {
        const currentUser = await getCurrentUser();
                ...
                for (const comment of comments.nodes) {
                    if (comment.body.includes('github.com') &&
                        ...
                        if (prUrlMatch) {
                            ...
                            const prClosedComment = comments.nodes.find(c => c.body.includes(`closed pull request #${prNumber}`) ||
