...
export async function setupGithub() {
    ...
    try {
        ...
        console.error(chalk.red('Error connecting to GitHub:'), error instanceof Error ? error.message : 'Invalid token');
 ...
 */
async function createSampleCommitIfNeeded(taskId, description) {
    try {
 ...
 */
export async function createBranch(branchName, baseBranch = config.get('defaultBranch') || 'development') {
    if (!isGitRepository()) {
 ...
 */
export async function createPullRequest(title, body, branch, baseBranch = config.get('defaultBranch') || 'development', isDraft = true) {
