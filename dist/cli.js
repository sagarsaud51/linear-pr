 ...
 */
async function setupLinearWithKey(apiKey) {
    try {
        ...
        console.error(chalk.red('Error connecting to Linear with provided API key:'), error instanceof Error ? error.message : 'Invalid token');
 ...
 */
async function setupGithubWithToken(token) {
    try {
        ...
        console.error(chalk.red('Error connecting to GitHub with provided token:'), error instanceof Error ? error.message : 'Invalid token');
...
async function selectFromAssignedTasks() {
    ...
    try {
        ...
        if (searchTerm && searchTerm.trim() !== '') {
            ...
            filteredTasks = tasks.filter(task => task.taskId.toLowerCase().includes(term) ||
